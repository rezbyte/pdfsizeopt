...
if not ((3, 0) <= sys.version_info[:2] < (4, 0)):
    sys.stderr.write("fatal: Python version 3.0 or greater needed for: %s\n" % __file__)
    sys.exit(1)

if os.path.isfile(
    os.path.join(os.path.dirname(__file__), "lib", "pdfsizeopt", "main.py")
):
...
class PdfSizeOptTest(unittest.TestCase):
    ...
    def testSerializePdfStringUnsafeAndParsePdfString(self):
        ...
        CheckParse(
            "(hi\\08\\18\\28\\38\\079\\179\\279\\379)",
        ...
        self.assertEqual(
            ("", 2), main.PdfObj.ParsePdfString("()/", 0, 3, is_partial_ok=True)
        )
        self.assertEqual(
        ...
        self.assertEqual(
            ("", 2), main.PdfObj.ParsePdfString("<>>>>>", is_partial_ok=True)
        )
        self.assertEqual(
            ("", 3), main.PdfObj.ParsePdfString("?<>>>>>", start=1, is_partial_ok=True)
        )
        self.assertEqual(
            ("\x8d\x50", 5), main.PdfObj.ParsePdfString("<8d5>]/", is_partial_ok=True)
        )
        self.assertRaisesX(
            main.PdfTokenTruncated,
            main.PdfObj.ParsePdfString,
            "<8d5",
            is_partial_ok=True,
        )
        self.assertRaisesX(
        ...
        self.assertRaisesX(
            main.PdfTokenParseError, main.PdfObj.ParsePdfString, "<\n3\t1\r4f5C5]>"
        )
        self.assertRaisesX(
            main.PdfTokenParseError, main.PdfObj.ParsePdfString, "<\n3\t1\r4f5C5]"
        )
        self.assertRaisesX(
            main.PdfTokenTruncated, main.PdfObj.ParsePdfString, "<\n3\t1\r4f5C5"
        )
    ...
    def testRewriteToParsable(self):
        ...
        self.assertEqual(" << true false null >>", e("% hi\r<<%\ntrue false null>>baz"))
        self.assertEqual(" << true >>", e("<<true>>baz"))
        self.assertEqual(" [ [ << [ << << >> >> ] >> ] ]", e("[[<<[<<<<>>>>]>>]]"))
        ...
        self.assertEqual(
            " <2829%s>" % "".join(["%02x" % {13: 10}.get(i, i) for i in range(33)]),
        ...
        self.assertEqual(
            " <0006073839050e170338043805380638073838380a3913391f39>",
        ...
        self.assertEqual(
            ...
            e(
                "(\\0\\6\\7\\8\\9\\05\\16\\27\\38\\48\\58\\68\\78\\88"
        ...
        self.assertEqual(
            " 5 endobz",
            e(" 5 endobz\t", end_ofs_out=end_ofs_out, do_terminate_obj=True),
        )
        ...
        self.assertRaisesX(main.PdfTokenTruncated, e, "true foo", do_terminate_obj=True)
        self.assertEqual(" <68656c296c6f0a0877286f72296c64>", e("(hel\)lo\n\bw(or)ld)"))
        self.assertRaisesX(main.PdfTokenTruncated, e, "(hel\)lo\n\bw(orld)")
        self.assertEqual(
            " [ <68656c296c6f0a0877286f72296c64> ]",
        ...
        self.assertRaisesX(
            main.PdfTokenTruncated, e, "<<>>stream\r", do_terminate_obj=True
        )
        ...
        self.assertEqual(
            " << /Type /Catalog /Pages 3 0 R >>", e("<</Type /Catalog /Pages 3 0 R\n>>")
        )
    ...
    def testSerializeDict(self):
        ...
        self.assertEqual(
            ...
            main.PdfObj.SerializeDict(
                main.PdfObj.ParseSimplestDict(
    ...
    def testIsGrayColorSpace(self):
        ...
        self.assertRaisesX(main.PdfTokenTruncated, e, "\t[ /Indexed /DeviceRGB 5 (]")
    ...
    def DoTestParseSimplestDict(self, e):
        ...
        self.assertRaisesX(
            main.PdfTokenParseError, e, "<</One/Two/Three Four/Five/Six>>"
        )
        self.assertEqual(
            {"Five": "/Six", "Three": "-4", "One": "/Two"},
            e("<</One/Two/Three -4.000/Five/Six>>"),
        )
        self.assertEqual(
            {"A": True, "C": None, "B": False, "E": "42.5", "D": 42},
            e("<<\n\r/A true/B\f\0false/C null/D\t42/E 42.5\r>>"),
        )
        self.assertEqual({"Data": "42 137 R"}, e("<</Data 42\t\t137\nR >>"))
        self.assertEqual(
            {"S": "<68656c6c6f2c20776f726c6421>"}, e("<</S(hello, world!)>>")
        )
    ...
    def testParseSimplestDict(self):
        ...
        self.assertRaisesX(main.PdfTokenNotSimplest, e, "<</Three[/Four()]>>")
    ...
    def testParseDict(self):
        ...
        self.assertEqual({"S": "<282929285c>", "T": 8}, e("<</S(()\\)\\(\\\\)/T 8>>"))
        ...
        self.assertEqual({"A": "[[]]", "q": "56 78 R"}, e("<</A[[]]/q\t56\r78%q\rR>>"))
    ...
    def testParseValueRecursive(self):
        ...
        self.assertEqual(
            "<29282868656c292929296c6f295c>", e("(\\)((hel\\)\\))\\)lo)\\\\)")
        )
        self.assertEqual([], e("[]"))
        self.assertEqual({}, e("<<>>"))
    ...
    def testCompressValue(self):
        ...
        self.assertEqual(
            uu, e(s, obj_num_map={43: 53, 42: 52}, do_emit_safe_strings=False)
        )
        old_obj_nums = [None]
        self.assertEqual(
        ...
        self.assertEqual(
            "<</Length 68/Filter/FlateDecode>>", e("<</Length 68/Filter/FlateDecode >>")
        )
        self.assertEqual(
            "<</Type/Catalog/Pages 1 0 R>>", e("<</Type/Catalog/Pages 1 0 R >>")
        )
        self.assertEqual("[/Zoo#3C#3E 1]", e("[/Zoo#3c#3e 1]"))
        self.assertEqual(
            "[/Zoo#3C#3E(a)/foo#2A/bar#2A#5B/pedal.#2A]",
            e("[/Zoo#3c#3e(a\\\n)/foo*/bar#2a#5b/pedal.#2a]"),
        )
        self.assertEqual(
            "[/Zoo#3C#3E<0a>/foo#2A/bar#2A#5B/pedal.#2A]",
            e("[/Zoo#3c#3e(\r\\\n)/foo*/bar#2a#5b/pedal.#2a]"),
        )
        self.assertEqual(
            "[/Zoo#3C#3E<0d>/foo#2A/bar#2A#5B/pedal.#2A]",
            e("[/Zoo#3c#3e(\\r\\\n)/foo*/bar#2a#5b/pedal.#2a]"),
        )
        self.assertEqual(
            "[/Zoo#3C#3E<0a>/foo*/bar*#5B/pedal.*]",
            e(
                "[/Zoo#3c#3e(\n\\\n)/foo*/bar#2a#5b/pedal.#2a]",
    ...
    def testPdfObjParse(self):
        obj = main.PdfObj("42 0 obj<</Length  3>>stream\r\nABC endstream endobj")
        ...
        self.assertEqual("<</Length 3>>", obj.head)
        self.assertEqual("ABC", obj.stream)
        obj = main.PdfObj("42 0 obj<</Length 4>>stream\r\nABC endstream endobj")
        self.assertEqual(
            "ABC ",
            main.PdfObj("42 0 obj<</Length 99>>stream\r\nABC endstream endobj").stream,
        )
        ...
        self.assertEqual("ABC", obj.stream)
        obj = main.PdfObj("42 0 obj<</T()/Length  3>>stream\nABC endstream endobj")
        self.assertEqual("ABC", obj.stream)
        ...
        self.assertRaisesX(
            main.PdfTokenParseError, main.PdfObj, "%s#%s" % (s, t), start=len(s)
        )
        ...
        obj = main.PdfObj("42 0 obj[/Foo%]endobj\n42  43\t]\nendobj")
        ...
        obj = main.PdfObj(
            "42 0 obj<</BitsPerComponent\n\n4\f" "/A ((\)\)endobj)x) >>\t\tendobj"
        )
        self.assertEqual("<</BitsPerComponent 4/A<282929656e646f626a2978>>>", obj.head)
        ...
        self.assertEqual("(@)", obj.head)
        self.assertRaisesX(
            main.PdfTokenParseError, main.PdfObj, "42 0 obj /foo# endobj"
        )
        self.assertRaisesX(
            main.PdfTokenParseError, main.PdfObj, "42 0 obj /foo#b endobj"
        )
        self.assertRaisesX(
            main.PdfTokenParseError, main.PdfObj, "42 0 obj /foo#bxar endobj"
        )
        self.assertRaisesX(
            main.PdfTokenParseError, main.PdfObj, "42 0 obj [()<a endobj"
        )
        self.assertRaisesX(
            main.PdfTokenParseError, main.PdfObj, "42 0 obj [()<g> endobj"
        )
        self.assertRaisesX(main.PdfTokenParseError, main.PdfObj, "42 0 obj [<a endobj")
        self.assertRaisesX(main.PdfTokenParseError, main.PdfObj, "42 0 obj [<g> endobj")
        self.assertRaisesX(main.PdfTokenParseError, main.PdfObj, "42 0 obj %\n\nendobj")
        end_ofs_out = []
        ...
        self.assertRaisesX(
            main.PdfTokenParseError,
            main.PdfObj,
            "42 0 obj<</Filter [/LZW /AHx]/Length 0>>" "stream%\nendstream endobj",
        )
        obj = main.PdfObj(
            "42 0 obj<</Filter [/LZW /AHx]/Length 0>>" "stream\nendstream endobj"
        )
        ...
        obj = main.PdfObj(
            "42 0 obj<</Filter [/LZW /AHx]/Length 0>>" "stream endstream endobj"
        )
        self.assertEqual(expected_head, obj.head)
        obj = main.PdfObj(
            "42 0 obj<</Filter [/LZW /AHx]/Length 0>>" "stream\r\nendstream endobj"
        ...
        self.assertEqual(expected_head, obj.head)
        obj = main.PdfObj(
            "42 0 obj<</Filter [/LZW /AHx]/Length 42" "/DecodeParms 43/Foo /Bar>>endobj"
        )
    ...
    def testParseTrailer(self):
        ...
        self.assertEqual("<</Root 4 0 R>>", F("trailer<</Root 4 0 R>>startxref ").head)
        self.assertEqual("<</Root 4 0 R>>", F("trailer<</Root 4 0 R>>xref ").head)
        end_ofs_out = []
        trailer = "footrailer\t<< /Root\n4\n0\nR\r>>\fstartxref "
        self.assertEqual(
            "<</Root 4 0 R>>", F(trailer, start=3, end_ofs_out=end_ofs_out).head
        )
        self.assertEqual("startxref ", trailer[end_ofs_out[0] :])
        end_ofs_out = []
        trailer = "footrailer\t<< /Root\n4\n0\nR/Hi(\\041>>xref )\r>>%hi\n\fxref\t"
        self.assertEqual(
            "<</Root 4 0 R/Hi<213e3e7872656620>>>",
    ...
    def testParseTokensToSafeSimple(self, is_simple_ok=True):
        ...
        self.assertEqual(
            "<</Length 68/Filter/FlateDecode>>", F("<</Length 68/Filter/FlateDecode >>")
        )
        self.assertEqual(
            "<</Type/Catalog/Pages 1 0 R>>", F("<</Type/Catalog/Pages 1 0 R >>")
        )
        ...
        self.assertEqual("<<true false null>>baz", F("% hi\r<<%\ntrue false null>>baz"))
        self.assertEqual("<<true>>baz", F("<<true>>baz"))
        self.assertEqual("[[<<[<<<<>>>>]>>]]", F("[[<<[<<<<>>>>]>>]]"))
        ...
        self.assertEqual("<68656c296c6f0a0877286f72296c64>", F("(hel\)lo\n\bw(or)ld)"))
        ...
        self.assertRaisesX(
            main.PdfTokenTruncated, F, " [ (hel\\051lo\\012\\010w\\050or\\051ld) ]<"
        )
        ...
        self.assertEqual("<<>>stream", F("<<>>stream\r\n"))
        self.assertEqual(
            "<</Type/Catalog/Pages 3 0 R>>", F("<</Type /Catalog /Pages 3 0 R\n>>")
        )
    ...
    def testFindEqclassesTwoGroupsByHead(self):
        ...
        for obj_num in new_objs:
            new_objs[obj_num] = (new_objs[obj_num].head, new_objs[obj_num].stream)
        self.assertEqual(
            {3: ("<</S(q)/P 4 0 R>>", None), 4: ("<</S(q)/Q 3 0 R>>", None)}, new_objs
        )
    ...
    def testFindEqclassesTwoGroupsByStream(self):
        ...
        for obj_num in new_objs:
            new_objs[obj_num] = (new_objs[obj_num].head, new_objs[obj_num].stream)
        self.assertEqual(
            {1: ("<</S(q)/P 2 0 R>>", None), 2: ("<</S(q)/P 1 0 R>>", "foo")}, new_objs
        )
    ...
    def testFindEqclassesTwoGroupsWithTrailerUnused(self):
        ...
        pdf.objs[12].stream = "blah"
        pdf.objs["trailer"] = pdf.trailer
        new_objs = main.PdfData.FindEqclasses(pdf.objs, do_remove_unused=True)
        del pdf.objs["trailer"]
        for obj_num in new_objs:
            new_objs[obj_num] = (new_objs[obj_num].head, new_objs[obj_num].stream)
        self.assertEqual(
    ...
    def testFindEqclassesCircularReferences(self):
        ...
        pdf.trailer = main.PdfObj("0 0 obj<<4 0 R 5 0 R 9 0 R 10 0 R>>endobj")
        ...
        self.assertEqual(
            {
                ...
                2: (
                    "<</Type/Page/MediaBox[0 0 419 534]/CropBox[0 0 419 534]"
                    "/Parent 1 0 R/Resources<</XObject<</S null>>"
                    "/ProcSet[/PDF/ImageB]>>/Contents null>>",
                    None,
                ),
                "trailer": ("<<1 0 R 1 0 R 2 0 R 2 0 R>>", None),
            },
            new_objs,
        )
    ...
    def testFindEqclassesString(self):
        pdf = main.PdfData()
        pdf.trailer = main.PdfObj("0 0 obj<</A[3 0 R]>>endobj")
        ...
        for obj_num in new_objs:
            new_objs[obj_num] = (new_objs[obj_num].head, new_objs[obj_num].stream)
        self.assertEqual(
            {"trailer": ("<</A[1 0 R]>>", None), 1: ("<</A()/B()/C(:)/D(::@)>>", None)},
            new_objs,
        )
    ...
    def testParseAndSerializeCffDict(self):
        cff_dict = {
            ...
            12004: [0],
            5: [0, -270, 812, 769],
            12007: ["0.001", 0, "0.000287", "0.001", 0, 0],
            15: [281],
            16: [245],
            17: [350],
            18: [21, 6489],
        ...
        cff_str1 = (
            "f81b01f81c02f81d038bfba2f9c0f99505f81e0c008b0c038b0c"
            "041e0a001f8b1e0a000287ff1e0a001f8b8b0c07a01c195912f7"
            "f211f7ad0ff78910".decode("hex")
        )
        cff_str2 = (
            "f81b01f81c02f81d038bfba2f9c0f99505f7ad0ff78910f7f211"
    ...
    def testParseCffDifferent(self):
        ...
        self.assertEqual({17: [410]}, cff.ParseCffDict("\x1d\x00\x00\x01\x9a\x11"))
        self.assertEqual({17: [410]}, cff.ParseCffDict("\xf8.\x11"))
        self.assertEqual("\xf8.\x11", cff.SerializeCffDict({17: [410]}))
    ...
    def testPermissiveZlibDecompress(self):
        ...
        self.assertEqual(
            data, e(compressed[:-5])
        )  # No Adler-32 checksum, no end-of-strem marker, output truncated.
    ...
    def testResolveReferencesChanged(self):
        ...
        self.assertEqual(
            ("<39203020525b205d>foo bar", True), e("(9 0 R[\\040])12 0 R", objs)
        )
        ...
        self.assertEqual(("<68656c6c6f>", True), e("41 0 R", objs, do_strings=True))
        self.assertEqual(
            (
                "<787878787878787878787878787878787878787878787878787878787878787878787878787878787878>",
                True,
            ),
    ...
    def testPdfToPsName(self):
        ...
        self.assertEqual("<666f6f7b7d>cvn", f("/foo#7b#7d", is_nonname_char_ok=True))
    ...
    def testMergeBaseEncodingToFontObj(self):
        ...
        def F(obj_str):
            ...
            main.PdfData._MergeBaseEncodingToFontObj(font_obj, base_encoding, objs)
            return font_obj.Get("Encoding")

        # Unchanged.
        self.assertEqual("/Foo", F("1 0 obj<</Encoding/Foo>>endobj"))
        self.assertEqual(
            "<</Differences[65/AAA]/BaseEncoding/WinAnsiEncoding>>",
            F("1 0 obj<<>>endobj"),
        )
        # Unchanged.
        self.assertEqual(
        ...
        # Unchanged.
        self.assertEqual(
            "<</BaseEncoding/Foo" "/Differences[66/BB/CC 100/dd 101/ee]>>",
            F(
                "1 0 obj<</Encoding<</BaseEncoding/Foo/Differences"
                "[66/BB/CC 100/dd  101/ee]>>>>endobj"
            ),
        )
        self.assertEqual(
            "<</Differences[65/AAA/BB/CC" "/D/E/F/G/H/I/J/K/L/M/N/O/P 100/dd/ee]>>",
            F("1 0 obj<</Encoding<</Differences" "[66/BB/CC 100/dd 101/ee]>>>>endobj"),
        )
    ...
    def testShellQuoteUnix(self):
        ...
        self.assertEqual("''", F(""))
        self.assertEqual(
            "/usr/BIN/i686-w64-mingw32-g++", F("/usr/BIN/i686-w64-mingw32-g++")
        )
        self.assertEqual("_.,:", F("_.,:"))
        self.assertEqual("'Hello, World!'", F("Hello, World!"))
        self.assertEqual(
            "'foo'\\''b'\\''ar\\baz\"x\\\"y\\\\'", F("foo'b'ar\\baz\"x\\\"y\\\\")
        )
        self.assertEqual("' '", F(" "))
        self.assertEqual("'\t'", F("\t"))
        ...
        self.assertEqual("'\\\\'", F("\\\\"))
        self.assertEqual(
            "'C:\\pdfsizeopt\\pdfsizeopt.exe'", F("C:\\pdfsizeopt\\pdfsizeopt.exe")
        )
        self.assertEqual("'|\\'", F("|\\"))
    ...
    def testRedirectOutputWindows(self):
        ...
        self.assertEqual(
            r'foo "bar & \\baz &&"&by>&2', F(r'foo "bar & \\baz &&"&by')
        )  # Too complicated.
        self.assertEqual(
            r"c1 2>f1 & c2 2>f2>&2", F(r"c1 2>f1 & c2 2>f2")
        )  # Too complicated.
        self.assertEqual("foo bar  >&2& baz  >&2&&>&2", F("foo bar  & baz  &&"))
        self.assertEqual(
            "foo bar   2>&1& baz   2>&1&& 2>&1", F("foo bar  & baz  &&", mode=True)
        )
        self.assertEqual(
            'c1  2>&1>f1 & c2  2>&1&& "&c3"  2>&1>f3',
            F('c1 >f1 & c2 && "&c3" >f3', mode=True),
        )
        self.assertEqual(
            "foo bar  >nul 2>&1& baz  >nul 2>&1&&>nul 2>&1",
            F("foo bar  & baz  &&", mode=None),
        )
        self.assertEqual(
            'c1  2>nul >f1 & c2 >nul 2>&1&& "&c3"  2>nul >f3',
            F('c1 >f1 & c2 && "&c3" >f3', mode=None),
        )
        self.assertEqual(
            'c1 >f1 & c2 >&2&& "&c3" >f3',  # Don't change c1 and c3.
            F('c1 >f1 & c2 && "&c3" >f3'),
        )
    ...
    def testYieldParsePostScriptTokenList(self):
        ...
        self.assertEqual(
            ["/pedal.#2A", "/pedal.#232a", "/pedal.#232A"],
        ...
        self.assertEqual(
            repr(["def", True, False, None]), repr(F("def true false null"))
        )
        self.assertEqual(
            repr([42, 5, -42, "425.", "-4.25"]), repr(F("42 +5 -42 +42.5e+1 -42.5E-1"))
        )
        self.assertEqual(
            repr(["def", True, "/false", "<>", None, 42]),
            repr(F("def true/false<>null%foo\r42%")),
        )
        self.assertEqual(
            ["<>", "<>", "<202a>", "<2a3b>"], F("()<\f\t>( *)<\r2 A\t3\nb\r>")
        )
    ...
    def testFixFontNameInType1C(self):
        ...
        self.assertEqual(
            new_font_name, cff.ParseCffHeader(font_obj.GetUncompressedStream())[1]
        )
    ...
    def testFixFontNameInCff(self):
        def CheckFont(font_program, font_name):
            ...
            self.assertEqual(
                self.CFF_FONT_PROGRAM_STRINGS, list(map(str, cff_string_bufs))
            )
