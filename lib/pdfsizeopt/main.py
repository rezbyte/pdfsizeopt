...
__pychecker__ = (
    "maxlines=999 maxlocals=99 unusednames=self,cls maxreturns=99 " "maxbranches=9999"
)
...
def VerifyGs(gs_cmd, is_verbose):
    ...
    lines = [
        line
        for line in lines
        if not line.startswith("Copyright ") and "NO WARRANTY" not in line
    ]
    data = "; ".join(lines)
                                ...
                                if re.match(r"gs[89][.]\d\d\Z", entry):
                                    ...
                                    if os.path.isfile(fn):
                                        ...
                                        if data:
                                            break
                                        LogInfo(
                                            "this Ghostscript does not work: %s"
                                            % gs_cmd
                                        )
                                        data = gs_cmd = None
                            if gs_cmd is not None:
...
def RedirectOutputWindows(cmd, mode=False):
    ...
    if (
        ...
        or " 2>" in cmd
        or (  # Too complicated.
            "&" not in cmd and "\n" not in cmd and (mode or ">" not in cmd)
        )
    ):  # Simple, appending will do.
        return cmd + suffix
    if mode is None:

        def replf(match):
            return (
                suffix * (match.group()[0] not in '">')
                + suffixb * (match.group()[0] == ">")
                + match.group()
            )
        ...
        def replf(match):
            return suffix * (match.group()[0] not in ics) + match.group()
...
def GetMultivalentCompressCommand(libexec_dir, _cache=[]):
    ...
    if compress_exe is not None:
        ...
        if libexec_dir is not None and os.path.isfile(
            os.path.join(libexec_dir, "avian")
        ):
        ...
        result = "%s -cp %s -Djava.awt.headless=true tool.pdf.Compress" % (
            ShellQuote(multivalent_java),
...
def FormatPercent(num, den):
    if den == 0:
        return "?%"
    return "%d%%" % int((num * 100 + (old_div(den, 2))) // den)


def FormatPercentTwoDigits(num, den):
    if den == 0:
        return "?%"
    v = (num * 10000 + (old_div(den, 2))) // den
    ...
    try:
        ...
        if os.path.exists(fromfn) and os.path.exists(tofn):
            try:
                ...
                pass
        LogFatal("unable to rename from %r to %r: %s" % (fromfn, tofn, e), 4)
...
def PermissiveZlibDecompress(data):
    ...
    try:
        ...
        # This won't work data = zlib.decompress(memoryview(data, 2), -wbits)
        # It may raise:
...
class PdfObj(object):
  ...
  """
    ...
    PDF_END_OF_REF_RE = re.compile(r"[\0\t\n\r\f ]R(?=[\0\t\n\r\f /%(<>\[\]]|\Z)")
  ...
  members of PDF_SAFE_KEEP_HEX_ESCAPED_RE,. """
    ...
    PDF_OPTIMIZED_KEEP_HEX_ESCAPED_RE = re.compile(r"[/#\[\]()<>{}\0\t\n\r\f %]")
  ...
  * # is considered unsafe so that we'd be able to do
  ...
  """
    ...
    PDF_STRING_UNSAFE_CHAR_RE = re.compile("[%s]" % re.escape(PDF_STRING_UNSAFE_CHARS))
  ...
  * \\ is considered unsafe because of (\)) .
  ...
  Useful for detecting PDF token sequence syntax errors in simple parsing."""
    ...
    PDF_HEX_STRING_LITERAL_OR_DICT_RE = re.compile(r"<(?:<|[\0\t\n\r\f 0-9a-fA-F]*>?)")
  ...
  """
    ...
    PDF_TOKENS_NONSIMPLE_CHAR_RE = re.compile(
        r"[^-+A-Za-z0-9_.#/\[\]<>\0\t\n\r\f "
        + PDF_UNSAFE_NAME_IN_SIMPLE_RE.pattern[1:-1]
        + r"]"
    )
  ...
  """
    ...
    PDF_TOKENS_INTERESTING_RE = re.compile(
        PDF_COMMENT_OR_WHITESPACE_RE.pattern
        + r"(?=([^\0\t\n\r\f ]|\Z))|"  # 1. Comment or whitespace.
        r"\(([^\\()\r]*)\)|"  # 2. Simple string: without parens or backslash.
        r"(\()|"  # 3. Beginning of a complicated string.
        r"(/[-+A-Za-z0-9_.]*[^<>(){}\[\]/\0\t\n\r\f %\-+A-Za-z0-9_.][^<>(){}\[\]/\0\t\n\r\f %]*)|"
        + r"(/(?=[<>(){}\[\]/\0\t\n\r\f %]|\Z))|"  # 4. Name with explicit hex (#AB) escape or name which needs hex-escaping. !!! Reuse PDF_SAFE_KEEP_HEX_ESCAPED_RE.
        + r"(#[0-9a-fA-F]{0,2})|"  # 5. An empty name token.
        + r"("  # 6. A hex-escape (usually in a name or a keyword).
        + PDF_HEX_STRING_LITERAL_OR_DICT_RE.pattern
        + ")|"  # 7. Hex string literal or stray <.
        r"([{}\\\v\)]|>>?)|"  # 8. Invalid PDF tokens (except for >>). (At least invalid outside name tokens.)
        + PDF_STREAM_OR_ENDOBJ_RE.pattern[:-1]
        + "|startxref[\0\t\n\r\f ]|xref[\0\t\n\r\f ])"
    )  # 9. stream or endobj or startxref or xref.
  ...
  """
    ...
    PDF_STARTXREF_EOF_AT_EOS_RE = re.compile(PDF_STARTXREF_EOF_RE.pattern + r"\Z")
  ...
  """
    ...
    PDF_PREFIXED_STARTXREF_RE = re.compile(
        ">>" + PDF_COMMENTS_OR_WHITESPACE_RE.pattern + r"(startxref|xref)[\0\t\n\r\f ]"
    )
  ...
  """
    ...
    PDF_WHITESPACE_OR_HEX_STRING_RE = re.compile(r"[\0\t\n\r\f ]+|(<<)|<(?!<)([^>]*)>")
  ...
  """
    ...
    PDF_COMMENT_OR_STRING_RE = re.compile(r"%[^\r\n]*|\(([^()\\]*)\)|(\()")
  ...
  """
    ...
    def __init__(
        ...
        try:
            ...
            raise e.__class__(
                "In obj data between ofs %d and %d: %s"
        ...
        if not match:
            ...
            for match in self.PDF_ENDSTREAM_ENDOBJ_RE.finditer(
                memoryview(other, stream_start_idx, len(other) - stream_start_idx)
            ):
                pass
            if match is None:
                raise PdfTokenParseError(
    ...
    @classmethod
    def ParseTokensToSafe(
        cls,
        data,
        start=0,
        end_ofs_out=None,
        do_expect_endobj=False,
        do_expect_startxref=False,
        is_simple_ok=True,
    ):
        ...
        if not is_simple_ok or cls.PDF_TOKENS_NONSIMPLE_CHAR_RE.search(
            data, start, end_for_simple
        ):
            ...
            while 1:
                ...
                if match.group(1) is not None:  # Whitespace or comment.
                    ...
                    if not (
                        output[-1][-1] in "<>[](){}/" or match.group(1) in "<>[](){}/"
                    ):
                        output.append(" ")
                    ...
                    if match.group() == ">>":
                        ...
                        raise PdfTokenParseError(
                            "Invalid PDF token: %r" % match.group()
                        )
                elif (
                    not match.start()
                    or data[match.start() - 1] not in "<>[](){}/\0\t\n\r\f "
                ):
                    ...
                    match.group().startswith("stream")
                    or match.group().startswith("endobj")
                ):
                    ...
                    match.group().startswith("startxref")
                    or match.group().startswith("xref")
                ):
                    if end_ofs_out is not None:
            ...
            def ReplacementAngle(match):
                ...
                if data == "<<":
                    return data
                if data[-1] != ">":
                    if match.end() == end and not (
                        do_expect_endobj or do_expect_startxref
                    ):
                        raise PdfTokenTruncated("Truncated hex string.")
                    else:
                        raise PdfTokenParseError("Invalid < token.")
                data = _whitespace_re.sub("", memoryview(data, 1, len(data) - 2))
                if len(data) & 1 != 0:
                    data += "0"
            ...
            data = memoryview(data, start, end_for_simple - start)
            ...
            for match in cls.PDF_ANGLE_BRACKET_FOR_SIMPLE_RE.finditer(data):
                ...
                if len(a) < 2 or a[-1] not in "<>":
                    if (
                        ...
                        and a[1:2] != "<"
                        and end == match.end()
                        and not (do_expect_endobj or do_expect_startxref)
                    ):
                        raise PdfTokenTruncated("Truncated hex string.")
                    else:
            ...
            if not (
                (data.startswith("<<") and data.find("<", 2) < 0) or data.find("<") < 0
            ):  # The `if' is just a shortcut for speed.
    ...
    def SetStreamAndCompress(
        self, data, may_keep_old=False, is_flate_ok=True, predictor_width=None, pdf=None
    ):
        ...
        if data:
            ...
            if predictor_width is not None and is_flate_ok:
                ...
                items[-1][2].Set(
                    "DecodeParms", "<</Predictor 12/Columns %d>>" % predictor_width
                )
                items[-1][0] = items[-1][2].size
                ...
                items[-1][2].Set(
                    ...
                    % (predictor_width, old_div(len(data), predictor_width)),
                )
                items[-1][0] = items[-1][2].size
        ...
        if items[0][2] is not self:
            ...
            if (
                pdf
                and items[0][1] == "zip-pred2"
                and predictor_width > 4
                and pdf.version < "1.3"
            ):
                pdf.version = "1.3"
    ...
    def ParsePdfString(cls, data, start=0, end=None, is_partial_ok=False):
        ...
        if data[start] == "<":
            ...
            data = cls.PDF_WHITESPACE_RE.sub(
                "", memoryview(data, start + 1, match.end() - 2 - start)
            )
            if (len(data) & 1) != 0:
                data += "0"
    ...
    def ParseDict(cls, data):
        ...
        if not cls.PDF_WHITESPACE_AT_EOS_RE.match(data, start, end):
            list_obj = cls._ParseTokens(
                data=data, start=start, end=end, count_limit=end
            )
            if 0 != (len(list_obj) & 1):
            ...
            for i in range(0, len(list_obj), 2):
                ...
                if not isinstance(key, str) or not key.startswith("/"):
                    ...
                    raise PdfTokenParseError(
                        "dict key expected, got %r... " % (str(key)[0:16])
                    )
    ...
    @classmethod
    def ParseTokenList(
        cls, data, count_limit=None, start=0, end=None, end_ofs_out=None
    ):
        ...
        if count_limit is None:
            count_limit = end
        return cls._ParseTokens(data, start, end, count_limit, end_ofs_out=end_ofs_out)
    ...
    @classmethod
    def _ParseTokens(cls, data, start, end, count_limit, end_ofs_out=None):
        ...
        while match:
            ...
            if value == "R":
                if (
                    len(list_obj) < 2
                    or not isinstance(list_obj[-1], int)
                    or list_obj[-1] < 0
                    or not isinstance(list_obj[-2], int)
                    or list_obj[-2] <= 0
                ):
                    raise PdfTokenParseError(
    ...
    def ParseXrefStreamWidths(cls, w_value):
        ...
        if (
            ...
            or [
                1
                for item in widths
                if not isinstance(item, int) or item < 0 or item > 10
            ]
            or widths[1] < 1
        ):
    ...
    def GetXrefStream(self, xref_ofs=None, xref_obj_num=None):
        ...
        if index_value is None:
            ...
            if (
                not index
                or len(index) % 2 != 0
                or [1 for item in index if not isinstance(item, int) or item < 0]
                or [1 for i in range(1, len(index), 2) if index[i] <= 0]
            ):
        ...
        xref_item_count = old_div(len(xref_data), sum(widths))
        if index_item_count != xref_item_count:
            msg = (
                "data length does not match /Index: "
                "xref_data_size=%d widths=%r index=%r" % (len(xref_data), widths, index)
            )
            ...
            if xref_item_count == index_item_count + 1:
                ...
                if w2:
                    f2 = PdfData.MSBFirstToInteger(
                        xref_data[i + w0 + w1 : i + w0 + w1 + w2]
                    )
                else:
                    f2 = 0
                if (
                    index
                    and index[-2] + index[-1] == xref_obj_num
                    and f0 == 1
                    and f1 == xref_ofs
                    and f2 == 0
                ):
    ...
    def GetAndClearXrefStream(self, xref_ofs, xref_obj_num):
        ...
        xref_tuple = self.GetXrefStream(xref_ofs=xref_ofs, xref_obj_num=xref_obj_num)
        self.stream = None
        self.Set("Type", None)
        self.Set("W", None)
        self.Set("Index", None)
    ...
    def CompressValue(
        ...
        def ReplacementWhiteOrString(match):
            ...
            if match.group(2) is not None:  # hex string
                ...
                if (
                    ...
                    or data[match.start() - 1] in "<>)[]{}"
                    or data[match.end()] in "/<>([]{}"  # % not needed.
                ):  # % not needed.
                    return ""
                else:
                    return " "
        ...
        return cls.PDF_WHITESPACE_OR_HEX_STRING_RE.sub(ReplacementWhiteOrString, data)
    ...
    def SerializePdfStringUnsafe(cls, data):
        ...
        while j < len(data):
            ...
            if (
                c == "\\"
                or (c == ")" and depth == 0)
                or (c == "(" and close_remaining <= depth)
            ):
    ...
    PDFDATA_IS_INDEXED_RGB_OR_GRAY_RE = re.compile(
        r"\[[\0\t\n\r\f ]*/Indexed[\0\t\n\r\f ]*/Device(?:RGB|Gray)" r"[\0\t\n\r\f ]+\d"
    )

    @classmethod
    def IsIndexedRgbOrGrayColorSpace(cls, colorspace):
        return colorspace and cls.PDFDATA_IS_INDEXED_RGB_OR_GRAY_RE.match(colorspace)
    ...
    def ParseRgbOrGrayPalette(cls, colorspace):
        ...
        if base == "RGB":
            ...
            palette = "".join(c + c + c for c in cls.ParsePdfString(match.group(2))[0])
            palette_size = 256 * 3
        else:
            assert 0, base
    ...
    def DetectInlineImage(self, objs=None):
        ...
        if (
            not re.search(r"/Subtype[\0\t\n\r\f ]*/Form\b", self.head)
            or not self.head.startswith("<<")
            or not self.stream is not None
            or self.Get("Subtype") != "/Form"
            or self.Get("FormType", 1) != 1
            or
            # !! get rid of these checks once we can decompress anything
            self.Get("Filter") not in (None, "/FlateDecode")
            or self.Get("DecodeParms") is not None
            or not str(self.Get("BBox")).startswith("[")
        ):
            return None
        ...
        if (
            ...
            or bbox[2] is None
            or bbox[2] < 1
            or bbox[2] != int(bbox[2])
            or bbox[3] is None
            or bbox[3] < 1
            or bbox[3] != int(bbox[3])
        ):
            return None
        width = int(bbox[2])
        height = int(bbox[3])
        ...
        inline_dict = PdfObj.CompressValue(match.group(3), do_emit_strings_as_hex=True)
        ...
        image_obj = PdfObj(
            "0 0 obj<</Length 0 %s>>stream endstream endobj"
            % self.ExpandAbbreviations(inline_dict)
        )
        if image_obj.Get("Width") != width or image_obj.Get("Height") != height:
            return None
  ...
  * PDF regular(40) character is any of [\\000-\\377] which is not whitespace
  ...
  """
        ...
        while stack:
            ...
            if o == 0:  # whitespace
                ...
                if (
                    number_match
                    or token[0] == "/"
                    or token in ("true", "false", "null", "R")
                ):
                    if token == "R" and (
                        len(output) < 3
                        or not re.match(r" -?\d+\Z", output[-2])
                        or not re.match(r" -?\d+\Z", output[-3])
                    ):
                ...
                if data[i] == "<":
                    ...
                    if not match or data[match.end() - 1] != ">":
                        ...
                        raise PdfTokenParseError("Bad hex string.")
                    j = match.end()
                    s = cls.PDF_WHITESPACE_RE.sub("", memoryview(data, i, j - 1 - i))
                ...
                raise PdfTokenParseError(
                    "syntax error, expecting PDF token, got %r" % data[i]
                )
    ...
    def GetUncompressedStream(self, objs=None):
        ...
        if (
            filter_value in ("/FlateDecode", "[/FlateDecode]")
            and "/Predictor" not in decodeparms
        ):
            try:
        ...
        gs_code = (
            ...
            "o closefile quit" % (filter_value, decodeparms_pair)
        )
        if sys.platform.startswith("win"):
            ...
            try:
                ...
                f.close()
            gs_defilter_cmd = "%s -dNODISPLAY -sINFN=%s -q -P- %s" % (
                GetGsCommand(),
                ShellQuoteFileName(tmp_file_name, is_gs=True),
                ShellQuoteFileName(ps_file_name, is_gs=True),
            )
        else:
            gs_defilter_cmd = "%s -dNODISPLAY -sINFN=%s -q -P- -c %s" % (
                GetGsCommand(),
                ShellQuoteFileName(tmp_file_name, is_gs=True),
                ShellQuote(gs_code),
            )
    ...
    def ResolveReferences(cls, data, objs, do_strings=False):
        ...
        if not (
            "R" in data
            and cls.PDF_REF_RE.search(data)  # cls.PDF_END_OF_REF_RE.search(data) and
        ):
        ...
        def Replacement(match):
            ...
            if obj.stream is None:
                ...
                if (
                    "R" in new_data
                    and cls.PDF_END_OF_REF_RE.search(new_data)
                    and cls.PDF_REF_RE.search(new_data)
                ):
                    ...
                    if "%" in new_data or "(" in new_data:  # ')'
                        new_data = cls.CompressValue(
                            new_data, do_emit_strings_as_hex=True
                        )
    ...
    def FixFontNameInType1C(self, new_font_name="F", objs=None, len_deltas_out=None):
    ...
    @classmethod
    def PdfRstripmemoryview(cls, data, start, end):
        ...
        return memoryview(data, start, end - start)
    ...
    def ParseObjStm(self, obj_num):
        ...
        Returns:
          ...
          second memoryview (Python) objects containing the head of each PDF object.
        Raises:
          PdfXrefStreamError:
          NotImplementedError:
        """
        ...
        rstrip_memoryview = self.PdfRstripmemoryview
        self = None  # Save memory.
        end_ofs_ary = []
        numbers = PdfObj.ParseTokenList(objstm_data, 2 * n, end_ofs_out=end_ofs_ary)
        ...
        compressed_obj_nums = []
        # List of (str) memoryview objects corresponding to the PDF token sequence
        ...
        for i in range(0, len(numbers), 2):
            ...
            if prev_offset > 0:
                compressed_obj_headbufs.append(
                    rstrip_memoryview(objstm_data, prev_offset, compressed_obj_ofs)
                )
            prev_offset = compressed_obj_ofs
        if prev_offset > 0:
            compressed_obj_headbufs.append(
                rstrip_memoryview(objstm_data, prev_offset, len(objstm_data))
            )
...
class ImageData(object):
    ...
    def __bool__(self):
        ...
        return bool(
            ...
            and self.height > 0
            and isinstance(self.bpc, int)
            and isinstance(self.is_inverted, bool)
            and self.bpc in (1, 2, 4, 8, 12, 16)
            and self.color_type
            in ("gray", "rgb", "indexed-rgb", "gray-alpha", "rgb-alpha")
            and self.compression in ("none", "zip", "zip-tiff", "zip-png", "jbig2")
            and isinstance(self.is_interlaced, bool)
            and self.is_interlaced in (True, False)
            and (self.color_type.startswith("indexed-") or self.plte is None)
            and isinstance(self.idat, str)
            and self.idat
            and (
                not self.color_type.startswith("indexed-")
                or (isinstance(self.plte, str) and len(self.plte) % 3 == 0)
            )
        )
    ...
    def CanBePdfImage(self):
        return bool(
            self
            and self.bpc in (1, 2, 4, 8)
            and self.color_type in ("gray", "rgb", "indexed-rgb")
            and not self.is_interlaced
        )
    ...
    def CanBePngImage(self, do_ignore_compression=False):
        ...
        return bool(
            ...
            and self.bpc in (1, 2, 4, 8)
            and self.color_type in ("gray", "rgb", "indexed-rgb")
            and (self.color_type != "rgb" or self.bpc == 8)
            and (do_ignore_compression or self.compression == "zip-png")
        )
        ...
        if self.color_type == "gray":
            ...
            for i in range(0, len(plte), 3):
                ...
                return "[/Indexed/DeviceGray %d%s]" % (
                    old_div(len(plte), 3) - 1,
                    PdfObj.SerializePdfStringSafe(
                        "".join(plte[i] for i in range(0, len(plte), 3))
                    ),
                )
            return "[/Indexed/DeviceRGB %d%s]" % (
                old_div(len(plte), 3) - 1,
                PdfObj.SerializePdfStringSafe(plte),
            )
        else:
    ...
    def GetPdfImageData(self):
        ...
        if self.compression == "none":
            ...
            if self.bpc != 8:
                ...
                prs_bpc = ""
            pdf_image_data["DecodeParms"] = "<<%s%s/Columns %d/Predictor %d>>" % (
                prs_colors,
                prs_bpc,
                self.width,
    ...
    def CanUpdateImageMask(self):
        ...
        if self.bpc != 1:
            return False
        if self.color_type == "indexed-rgb" and self.plte in (
            "\0\0\0\xff\xff\xff",
            "\0\0\0",
            "\0\0\0\0\0\0",
            "\xff\xff\xff\0\0\0",
    ...
    def UpdatePdfObj(self, pdf_obj, do_check_dimensions=True):
        ...
        if do_check_dimensions:
            assert (
                pdf_obj.Get("Width") == pdf_image_data["Width"]
            ), "image Width mismatch: %r vs %r" % (pdf_obj.head, pdf_image_data)
            assert (
                pdf_obj.Get("Height") == pdf_image_data["Height"]
    ...
    def CompressToZipPng(
        ...
        if self.compression == "zip-png":
            ...
            raise FormatUnsupported("cannot compress %s to zip-png" % self.compression)
    ...
    def SavePng(self, file_name, do_force_gray=False):
        ...
        if do_force_gray:
            assert self.color_type.startswith("indexed-") or self.color_type == "gray"
    ...
    def LoadPdf(self, f):
        ...
        image_obj_nums = [
            obj_num
            for obj_num in sorted(pdf.objs)
            if re.search(r"/Subtype[\0\t\n\r\f ]*/Image\b", pdf.objs[obj_num].head)
        ]
        ...
        if obj.Get("ImageMask"):
            ...
            page_objs = [
                pdf.objs[obj_num]
                for obj_num in sorted(pdf.objs)
                if re.search(r"/Type[\0\t\n\r\f ]*/Page\b", pdf.objs[obj_num].head)
                and pdf.objs[obj_num].head.startswith("<<")
                and pdf.objs[obj_num].Get("Type") == "/Page"
            ]
            ...
            color1 = (
                chr(int(float(match.group(1)) * 255 + 0.5))
                + chr(int(float(match.group(2)) * 255 + 0.5))
                + chr(int(float(match.group(3)) * 255 + 0.5))
            )
            color2 = (
                chr(int(float(match.group(4)) * 255 + 0.5))
            ...
            colorspace = "[/Indexed/DeviceRGB %d%s]" % (
                old_div(len(palette), 3) - 1,
                PdfObj.SerializePdfStringSafe(palette),
            )
    ...
    def LoadPdfImageObj(self, obj, do_zip, decode_kind=None):
        ...
        if decode_kind is None:
            decode_kind = PdfObj.ClassifyImageDecode(
                obj.Get("Decode"), int(palette is not None and bpc)
            )
    ...
    def LoadPng(self, f):
        ...
        while 1:
            ...
            if not (
                chunk_type in ("IHDR", "IDAT", "IEND")
                or (chunk_type == "PLTE" and need_plte)
            ):
                ...
                try:
                    ...
                    if f.tell() != target_ofs:
                        raise IOError("could not seek from %s to %s" % target_ofs)
                except IOError:
                    # Can't seek => read.
...
class PdfData(object):
    __slots__ = [
        ...
        "file_name",
        "file_size",
        "do_ignore_generation_numbers",
        "has_generational_objs",
    ]
    ...
    def Load(
        ...
        match = PdfObj.PDF_VERSION_HEADER_RE.search(memoryview(data, 0, 256))
        if not match:
        ...
        try:
            ...
            if "trailer" in obj_starts:
                ...
                obj_count -= 1
            LogInfo(
                "separated to %s objs%s" % (obj_count, obj_count_extra), is_proportional
            )
            ...
            raise NotImplementedError(
                ...
                % (
                    ...
                    ShellQuoteFileName(
                        os.path.splitext(self.file_name)[0] + ".decrypted.pdf"
                    ),
                )
            )
        ...
        for i in range(1, len(obj_items)):
            ...
            obj_data = memoryview(data, start_ofs, obj_items[i][0] - start_ofs)
            ...
            if _pdf_obj_def_re.match(obj_data):
                ...
                LogWarning(
                    ...
                    "got %r at ofs=%s" % (obj_num, obj_data[:32], start_ofs)
                )
            ...
            if not ii_remaining:
                ...
                if obj_num is not None and index[ii] <= obj_num:
                    ...
                    raise PdfXrefStreamError(
                        ...
                        "old_obj_num=%d new_obj_num=%d" % (obj_num, index[ii])
                    )
                obj_num = index[ii]
    ...
    def ParseUsingXrefStream(
        ...
        while 1:
            ...
            for obj_num, f0, f1, f2 in cls.YieldXrefStreamEntries(
                w0, w1, w2, index, xref_data
            ):
                ...
                if f0 == 1:  # f1 is the object offset in the file.
                    ...
                    if f1 < 9:
                        ...
                        if f1:
                            raise PdfXrefStreamError(
                                "offset of obj %d too small: %d" % (obj_num, f1)
                            )
                    else:
            ...
            if not match:
                raise PdfXrefStreamError(
                    "could not find obj at /Prev at %d: %d" % (xref_ofs, prev)
                )
            xref_ofs = prev
        ...
        for xref_obj_num in sorted(xref_obj_nums):
            ...
            if obj_start is None:
                if max_obj_num is None:
                    max_obj_num = max(
                        (obj_num != "xref" and obj_num != "trailer" and obj_num or 0)
                        for obj_num in obj_starts
                    )
                if xref_obj_num != max_obj_num + 1:
                    # pgfmanual.pdf in
                    # https://code.google.com/p/pdfsizeopt/issues/detail?id=75
                    LogWarning("missing offset for xref stream obj %d" % xref_obj_num)
            else:
                if not isinstance(obj_start, int):
                    LogWarning("in-object-stream xref stream obj %d" % xref_obj_num)
        ...
        for objstm_obj_num in sorted(obj_streams):
            ...
            if not isinstance(obj_start, int):
                raise PdfXrefStreamError(
                    "In-object-stream obj stream %d" % objstm_obj_num
                )
            try:
                ...
                if e.length_obj_num not in obj_starts:
                    raise PdfXrefStreamError(
                        "Parse objstm obj %d: %s" % (objstm_obj_num, e)
                    )
        ...
        for obj_num in sorted(obj_starts):
            ...
            if not isinstance(obj_start, int):
                ...
                compressed_obj_nums, compressed_obj_headbufs = obj_streams[
                    objstm_obj_num
                ]
        ...
        unused_obj_items = []
        for objstm_obj_num in sorted(obj_streams):
            unused_obj_count = sum(
                1 for j in obj_streams[objstm_obj_num][0] if j is not None
            )
            if unused_obj_count:
        ...
        if all_unused_obj_count:
            LogProportionalInfo(
                ...
                % (
                    ...
                    ", ".join(
                        "%d in objstm obj %d" % (b, a) for a, b in unused_obj_items
                    ),
                )
            )
    ...
    def ParseUsingXref(cls, data, do_ignore_generation_numbers):
        ...
        if match:
            xref_obj_num = int(match.group(1))
            xref_generation = int(match.group(2))
            return cls.ParseUsingXrefStream(
                data,
                do_ignore_generation_numbers,
                xref_ofs,
                xref_obj_num,
        ...
        while 1:
            ...
            while 1:
                ...
                while obj_count > 0:
                    ...
                    if not match:
                        raise PdfXrefError(
                            "syntax error in xref entry at %s" % xref_ofs
                        )
                    if match.group(3) == "n":
            ...
            if xrefstm_ofs is not None:  # Hybrid.
                if type(xrefstm_ofs) not in (int, int):
                    raise PdfXrefError(
                        "/XRefStm offset not an int: %r" % (xrefstm_ofs,)
                    )
                match = PdfObj.PDF_OBJ_DEF_RE.match(data, xrefstm_ofs)
                if not match:
                    raise PdfXrefStreamError(
                        "/XRefStm obj definition expected at %d" % xrefstm_ofs
                    )
                xrefstm_obj_num = int(match.group(1))
                xrefstm_obj_generation = int(match.group(2))
                xrefstm_objs.append(
                    (xrefstm_ofs, xrefstm_obj_num, xrefstm_obj_generation)
                )
            xref_ofs = trailer_obj.Get("Prev")
            del trailer_obj  # Save memory.
            if xref_ofs is None:
                break
            if type(xref_ofs) not in (int, int):
        ...
        if xrefstm_objs:
            ...
            for xrefstm_ofs, xrefstm_obj_num, xrefstm_obj_generation in xrefstm_objs:
                ...
                cls.ParseUsingXrefStream(
                    data,
                    do_ignore_generation_numbers,
                    xrefstm_ofs,
                    xrefstm_obj_num,
                    xrefstm_obj_generation,
                    obj_starts_copy,
                    do_allow_duplicate_obj=True,
                )
    ...
    def GenerateXrefStream(
        ...
        if objstm_obj_numbers:
            ...
            assert (
                len(obj_numbers) == obj_numbers_size + len(objstm_obj_numbers) + 1
        ...
        if obj_numbers[0] != 0:  # /Index [0 Size] is the default.
            ...
            # default).
            index_data = "[%d %d]" % (obj_numbers[0], max_obj_num - obj_numbers[0] + 1)
        ...
        for i in range(1, len(obj_numbers)):
            if obj_numbers[i] - 1 != obj_numbers[i - 1]:
                if not (
                    obj_numbers[i] - 2 == obj_numbers[i - 1]
                    and obj_numbers[i] - 1 == trailer_obj_num
                ):
                    need_w0 = True
        ...
        if max_ofs_size > 8:
            raise NotImplementedError(
                "unsupported max_ofs_size=%d for max_ofs=%d" % (max_ofs_size, max_ofs)
            )
        if need_w0:
            ...
            if index_size:
                ...
                if obj_numbers[0] <= old_div((index_size - 1), max_ofs_size):
        ...
        trailer_obj.SetStreamAndCompress(
            data, predictor_width=(max_ofs_size + extra_width), is_flate_ok=is_flate_ok
        )
    ...
    def AppendSerializedPdf(
        ...
        if do_generate_object_stream:
            ...
            for obj_num in obj_numbers:
                ...
                if pdf_obj.stream is None:
                    ...
                    if not (head.endswith("R") and PdfObj.PDF_REF_END_RE.search(head)):
                        if do_emit_short_unsafe:
                            # To Call this in the final objstm_output instead,
                            # we'd have to fix the offsets (objstm_numbers). See also
            ...
            if objstm_size:
                ...
                objstm_output[1] = " " * (
                    len(objstm_output) > 2
                    and PdfObj.IsSpaceNeeded(objstm_output[0], objstm_output[2])
                )
        ...
        for obj_num in obj_numbers:
            ...
            pdf_obj.AppendTo(output, obj_num, do_emit_short_unsafe=do_emit_short_unsafe)
        ...
        if objstm_obj:
            ...
            objstm_obj.AppendTo(
                output, objstm_obj_num, do_emit_short_unsafe=do_emit_short_unsafe
            )
        else:
            objstm_obj_num = None
        ...
        if do_generate_xref_stream:  # Emit xref stream containing trailer.
            ...
            self.GenerateXrefStream(
                obj_numbers=obj_numbers,
                obj_ofs=obj_ofs,
                xref_ofs=xref_ofs,
                trailer_obj=trailer_obj,
                trailer_obj_num=trailer_obj_num,
                objstm_obj_num=objstm_obj_num,
                objstm_obj_numbers=objstm_obj_numbers,
                is_flate_ok=is_flate_ok,
            )
            trailer_obj.AppendTo(
                output, trailer_obj_num, do_emit_short_unsafe=do_emit_short_unsafe
            )
    ...
    def GetFonts(self, font_type=None, do_obj_num_from_font_name=False, where="loaded"):
        ...
        good_font_file_tag = {"Type1": "FontFile", "Type1C": "FontFile3", None: None}[
            font_type
        ]
        ...
        for obj_num in sorted(self.objs):
            ...
            if (
                ...
                and "/FontFile" in obj.head
                and "/Flags" in obj.head  # /FontFile, /FontFile2 or /FontFile3
                and obj.head.startswith("<<")
            ):
                ...
                if font_file_count != 1:
                    continue
                if (
                    good_font_file_tag is not None
                    and font_file_dict.get(good_font_file_tag) is None
                ):
                    continue
                font_file_tag, font_file_value = next(
                    (  # Get the only non-None value.
                ...
                assert str(subtype).startswith(
                    "/"
                ), "expected font /Subtype, got %r in obj %s" % (subtype, font_obj_num)
        ...
        assert not duplicate_count, "found %d duplicate font objs %s" % (
            duplicate_count,
            where,
        )
        return objs
    ...
    @classmethod
    def GenerateType1CFontsFromType1(
        cls, objs, ref_objs, ps_tmp_file_name, pdf_tmp_file_name
    ):
        ...
        gs_cmd = (
            ...
            % (
                GetGsCommand(),
                ShellQuoteFileName(pdf_tmp_file_name, is_gs=True),
                ShellQuoteFileName(ps_tmp_file_name, is_gs=True),
            )
        )
        LogInfo("executing Type1CConverter with Ghostscript: %s" % gs_cmd)
        sys.stdout.flush()
        ...
        if not os.path.isfile(pdf_tmp_file_name):
            LogFatal("Type1CConverter has not created output: %s" % pdf_tmp_file_name)
        pdf = PdfData().Load(
            pdf_tmp_file_name, is_no_objs_ok=True, is_parse_error_ok=False
        )
        # TODO(pts): Better error reporting if the font name is wrong.
        type1c_objs = pdf.GetFonts(do_obj_num_from_font_name=True, where="in GS output")
    ...
    @classmethod
    def ParseType1CFonts(
        cls, objs, ps_tmp_file_name, data_tmp_file_name, is_permissive=False
    ):
        ...
        LogInfo("executing Type1CParser with Ghostscript: %s" % gs_cmd)
        sys.stdout.flush()
        ...
        assert sorted(parsed_fonts) == sorted(objs), "Data object number list mismatch."
        ...
        def MoveToPrivate(parsed_font, key):
            ...
            if parsed_font[key] == private_dict[key]:
                ...
                return
            raise ValueError(
                "Conflicting value in /Private and top for key /%s in CFF font." % key
            )
        ...
        for obj_num in sorted(objs):
            ...
            if not parsed_font:
                ...
                continue
            if (
                parsed_font.get("FontType") != 2
                or not isinstance(parsed_font.get("CharStrings"), dict)
                or "FontMatrix" not in parsed_font
                or "PaintType" not in parsed_font
            ):
                del parsed_fonts[obj_num]
                del objs[obj_num]
                LogWarning(
                    ...
                    % (
                        obj_num,
                        type(parsed_font.get("CharStrings")),
    ...
    def ConvertType1FontsToType1C(self):
        ...
        type1c_objs, encodings = self.GenerateType1CFontsFromType1(
            self.GetFonts("Type1"),
            self.objs,
            TMP_PREFIX + "conv.tmp.ps",
            TMP_PREFIX + "conv.tmp.pdf",
        )
        for obj_num in type1c_objs:
        ...
        if encodings:
            for obj_num in sorted(self.objs):
                ...
                if (
                    ...
                    and "/Type1" in head
                    and "/Subtype" in head
                    and "/FontDescriptor" in head
                    and obj.Get("Type") == "/Font"
                    and obj.Get("Subtype") == "/Type1"
                ):
    ...
    def FormatEncoding(cls, encoding):
        ...
        if outputs[0] == "<<>>":  # All /.notdef.
            return None
        outputs.extend(
            FormatWithBaseEncoding(*item) for item in sorted(PDF_FONT_ENCODINGS.items())
        )
    ...
    def _MergeBaseEncodingToFontObj(cls, font_obj, base_encoding, objs):
        ...
        encoding_value = font_obj.ResolveReferences(font_obj.Get("Encoding"), objs=objs)
                ...
                if isinstance(differences, str) and differences.startswith("["):
                    ...
                    if differences:
                        ...
                        if not (isinstance(i, int) and 0 <= i < 256):
                            raise ValueError(
                                "Bad initial index in /Differences: %r" % i
                            )
    ...
    @classmethod
    def SerializeType1CFonts(
        cls, parsed_fonts, target_objs, objs, do_double_check_type1c_output
    ):
        ...
        LogInfo(
            ...
            % (len(output_str) - output_prefix_len, len(parsed_fonts), ps_tmp_file_name)
        )
        f = open(ps_tmp_file_name, "wb")
        try:
            f.write(output_str)
        ...
        if not os.path.isfile(pdf_tmp_file_name):
            LogFatal("Type1CGenerator has not created output: %s" % pdf_tmp_file_name)
        pdf = PdfData().Load(
            pdf_tmp_file_name, is_no_objs_ok=True, is_parse_error_ok=False
        )
        # TODO(pts): Better error reporting if the font name is wrong.
        loaded_objs = pdf.GetFonts(do_obj_num_from_font_name=True)
        assert sorted(loaded_objs) == sorted(
            parsed_fonts
        ...
        if do_double_check_type1c_output:
            ...
            for obj_num in sorted(loaded_objs):
                ...
                assert cs == cs2, "missing glyphs from font %s: %r --> %r" % (
                    objs[obj_num].Get("FontName"),
                    cs,
                    cs2,
                )
    ...
    def _ProcessType1CFonts(
        ...
        parsed_fonts = self.ParseType1CFonts(
            objs=type1c_objs,
            ps_tmp_file_name=TMP_PREFIX + "conv.parse.tmp.ps",
        ...
        for obj_num in sorted(self.objs):
            ...
            if (
                ...
                and obj.Get("Type") == "/Font"
                and obj.Get("Subtype") == "/Type1"
                and self.IsFontBuiltInEncodingUsed(
        ...
        for obj_num in sorted(parsed_fonts):  # !! sort by font name to get exacts
            ...
            if "Subrs" in parsed_font.get("Private", ()):
                ...
                # /Subrs.
                LogProportionalInfo(
                    "not merging Type1C font obj %d because it has /Subrs" % obj_num
                )
                continue
        ...
        for font_group in sorted(font_groups):
            ...
            if len(all_glyphs) > 256:
                ...
                LogProportionalInfo(
                    ...
                    "not merging objs: %s" % (len(all_glyphs), group_obj_nums)
                )
                del font_groups[font_group]
                continue
            # Example: lme_v6.pdf
            ...
            encoding_obj_nums = [
                ...
                and not [
                    1
                    for font_obj_num in copy_encoding_dict[obj_num][1]
    ...
    def OptimizeType1CFonts(
        ...
        for key in h:
            ...
            for data_len, obj_num in same_type1c_objs:
                ...
                assert (
                    target_obj_num is not None
                    and self.objs[target_obj_num] is type1c_objs[obj_num]
    ...
    def ConvertImage(
        ...
        if "%(sourcefnq)s" not in cmd_pattern:
            ...
            f = open(sourcefn, "rb")
            try:
                data = f.read()  # TODO(pts): Use less memory for memoryviews.
            finally:
                f.close()
            f = open(targetfn, "wb")
            try:
        ...
        LogProportionalInfo("executing image converter %s: %s" % (cmd_name, cmd))
        sys.stdout.flush()
        romode = (None, False)[NeedToolLogOutput()]
        status = os.system(RedirectOutput(cmd, mode=romode))
        if return_none_if_status is not None and status == return_none_if_status:
            EnsureRemoved(targetfn)
            return None
        if status:
            LogFatal("image converter %s failed (status=0x%x)" % (cmd_name, status))
        assert os.path.exists(targetfn), "%s has not created the output image %r" % (
            cmd_name,
            targetfn,
        )
        if do_just_read:
    ...
    def ConvertInlineImagesToXObjects(self):
        ...
        for obj_num in sorted(self.objs):
            ...
            assert (
                image_obj.Get("BitsPerComponent") is not None
                or image_obj.Get("ImageMask", False) is True
            )
            ...
            resources_obj = PdfObj("0 0 obj %s endobj" % obj.Get("Resources", "<<>>"))
        ...
        if uninline_count > 0:
            ...
            # 50 bytes.
            LogInfo(
                "uninlined %s images, saved %s bytes"
    ...
    def RenderImages(cls, objs, ps_tmp_file_name, png_tmp_file_pattern, gs_device):
        ...
        for obj_num in sorted_objs:
            ...
            if "/JBIG2Decode" in str(obj.Get("Filter")) and "/JBIG2Globals" in str(
                obj.Get("DecodeParms")
            ):
            ...
            if "/CCITTFaxDecode" in str(obj.Get("Filter")) and "/BlackIs1" in str(
                obj.Get("DecodeParms")
            ):
                ...
                if [1 for parm in decodeparms if parm.get("BlackIs1") is True]:
                    ...
                    if decode_kind == "inverted":
                        ...
                        obj.Set(
                            "Decode",
                            PdfObj.GenerateImageDecode(
                                True, samples_per_pixel, indexed_bpc
                            ),
                        )
            if obj.Get("Decode") is None and "/Indexed" in str(obj.Get("ColorSpace")):
                obj = PdfObj(obj)
                ...
                obj.Set(
                    "Decode",
                    PdfObj.GenerateImageDecode(False, 1, obj.Get("BitsPerComponent")),
                )
        ...
        gs_cmd = "%s -q -P- -dNOPAUSE -dBATCH -sDEVICE=%s " "-sOutputFile=%s -f %s" % (
            GetGsCommand(),
            ShellQuote(gs_device),
            ShellQuoteFileName(png_tmp_file_pattern, is_gs=True),
            ShellQuoteFileName(ps_tmp_file_name, is_gs=True),
        )
        LogInfo("executing ImageRenderer with Ghostscript: %s" % gs_cmd)
        write_func = (lambda m: 0, sys.stderr.write)[NeedToolLogOutput()]
        sys.stdout.flush()
        ...
        assert not os.path.exists(
            png_tmp_file_pattern % (len(objs) + 1)
        ...
        for obj_num in sorted_objs:
            ...
            if not os.path.isfile(png_tmp_file_name):
                LogFatal("ImageRenderer has not created output: %s" % png_tmp_file_name)
    ...
    def _IsSlowCmdName(cls, cmd_name):
        return (
            "pngout" in cmd_name
            or "zopflipng" in cmd_name
            or "optipng" in cmd_name
            or "ect" in cmd_name
            or "advpng" in cmd_name
            or "pngwolf" in cmd_name
        )

    def _ConvertImageWithJbig2(self, image, cmd_name, cmd_pattern, obj_num, color_type):
        ...
        if color_type != "gray":
            image.SavePng(  # Changes .file_name.
                file_name=TMP_PREFIX + "img-%d.gray.png" % obj_num, do_force_gray=True
            )
    ...
    def OptimizeImages(self, img_cmd_patterns, do_fast_bilevel_images):
        ...
        for obj_num in sorted(self.objs):
            obj = self.objs[obj_num]
            if (
                not obj.head.startswith("<<")
                or "/Image" not in obj.head
                or not re.search(r"/Subtype[\0\t\n\r\f ]*/Image\b", obj.head)
                or not obj.stream is not None
            ...
            decode_kind = PdfObj.ClassifyImageDecode(
                obj.Get("Decode"), int("/Indexed" in colorspace and bpc)
            )
            if decode_kind not in ("normal", "inverted"):
                LogWarning(
            ...
            LogProportionalInfo(
                ...
                % (
                    ...
                    colorspace_short,
                    bpc,
                    decode_kind == "inverted",
                    obj.Get("Filter"),
                    int(bool(obj.Get("DecodeParms"))),
                    obj.size,
                    gs_device,
                )
            )
        ...
        if not images:  # No images => no conversion.
            return self
        LogInfo(
            "optimizing %s images of %s bytes in total"
        ...
        for gs_device in sorted(device_image_objs):
            ...
            if objs:
                ...
                rendered_images = self.RenderImages(
                    objs=objs,
                    ps_tmp_file_name=ps_tmp_file_name,
                    gs_device=gs_device,
                    png_tmp_file_pattern=TMP_PREFIX
                    + "img-%%04d.%s.tmp.png" % gs_device,
                )
                os.remove(ps_tmp_file_name)
        ...
        for obj_num in sorted(images):
            ...
            if target_image is not None:  # We have already rendered this image.
                ...
                LogProportionalInfo("using already rendered image for obj %s" % obj_num)
                ...
                do_fast_bilevel_images
                and has_jbig2
                and obj_images[-1][1].bpc == 1
                and obj_images[-1][1].color_type in ("gray", "indexed-rgb")
                and obj_images[-1][1].CanCompressToZipPng()
                and (obj_width * obj_height) >> 16
            ):  # Shortcut to do jbig2 only.
                oi_image = obj_images[-1][1]
                if not (
                    oi_image.color_type == "gray"
                    and oi_image.compression == "zip-png"
                    and (oi_image.file_name or "").endswith(".png")
                ):
                ...
                # grayscale output.
                assert (
                    obj_num not in force_grayscale_obj_nums
                    or np_image.color_type == "gray"
                ), ("Grayscale needed for image, got %s" % np_image.color_type)
                is_bilevel_image = np_image.bpc == 1 and np_image.color_type in (
                    "gray",
                    "indexed-rgb",
                )
                ...
                if (
                    do_fast_bilevel_images
                    and is_bilevel_image
                    and has_jbig2
                    and (obj_width * obj_height) >> 14
                ):
                    ...
                    if sam2p_pr_pattern is None or do_save_oi_fast:
                        ...
                        obj_images.append(
                            (
                                ...
                                ImageData(np_image)
                                .CompressToZipPng(
                                    do_try_invert=np_image.is_inverted,
                                    effort=(9 - 6 * do_save_oi_fast),
                                )
                                .SavePng(
                                    file_name=TMP_PREFIX
                                    + "img-%d.save-oi.png" % obj_num
                                ),
                            )
                        )
            ...
            for cmd_name, image_data in obj_images:
                if obj.Get("ImageMask") and not image_data.CanUpdateImageMask():
                    ...
                    continue
                if (
                    obj_num in force_grayscale_obj_nums
                    and image_data.color_type != "gray"
                ):
                    if cmd_name != "gs":
                        LogWarning(
                            "skipping bpc=%s color_type=%s file_name=%r "
                ...
                obj_infos.append(
                    (new_obj.size, cmd_name, image_data.file_name, new_obj, image_data)
                )
            ...
            if obj_infos[0][4] is None:
                ...
                LogProportionalInfo(
                    ...
                    "replacements too large: %s" % (obj_num, method_sizes)
                )
            else:
                ...
                LogProportionalInfo(
                    ...
                    % (
                        ...
                        obj_infos[0][2],
                        obj_infos[0][0],
                        FormatPercent(obj_infos[0][0], obj.size),
                        method_sizes,
                    )
                )
                bytes_saved += self.objs[obj_num].size - obj_infos[0][0]
                if (
                    "/JBIG2Decode" in (obj_infos[0][3].Get("Filter") or "")
                    and self.version < "1.4"
                ):
                    self.version = "1.4"
                ...
                if (
                    obj_num in force_grayscale_obj_nums
                    and obj.Get("ColorSpace") != "/DeviceGray"
                ):
            ...
            if obj_infos[0][4] is not None:
                by_rendered_tuple[rendered_tuple] = by_image_tuple[
                    image_tuple
                ] = obj_infos[0][4]
    ...
    @classmethod
    def FindEqclasses(
        cls, objs, do_remove_unused=False, do_renumber=False, do_unify_pages=True
    ):
        ...
        for obj_num in sorted(objs):
            ...
            if isinstance(obj_num, str):  # for 'trailer'
                ...
                not do_unify_pages
                and stream is None
                and head_minus.startswith("<<")
                and objs[obj_num].Get("Type") == "/Page"
            ):
            ...
            for eqclass in eqclasses:
                ...
                if len(eqclass) > 1:
                    ...
                    for i in range(1, len(eqclass)):
                        ...
                        refs_tob = descb[3]
                        j = 0
                        while j < len(refs_to) and eqclass_of.get(
                            refs_to[j]
                        ) is eqclass_of.get(refs_tob[j]):
                            j += 1
                        if j == len(refs_to):
                            eqlist.append(descb)
                        else:
        ...
        if do_renumber:
            ...
            def CompareDesc(desc_a, desc_b):
                ...
                return desc_b[4].__cmp__(desc_a[4]) or desc_a[  # inrefs_count
                    0
                ].__cmp__(
                    desc_b[0]
                )  # original obj_num
            ...
            descs = [
                eqclass[0]
                for eqclass in eqclasses
                if not isinstance(eqclass[0][0], str)
                and eqclass[0][0] not in unused_obj_nums
            ]
        ...
        for eqclass in eqclasses:
            ...
            head = PdfObj.PDF_HEX_STRING_OR_DICT_RE.sub(
                lambda match: (
                    ...
                    and PdfObj.SerializePdfStringSafe(match.group(1).decode("hex"))
                    or "<<"
                ),
                head,
            )
    ...
    def OptimizeStreams(self, do_decompress_only=False):
        ...
        for obj_num in sorted(self.objs):
            ...
            if obj.HasUncompressedStream():
                ...
                try:
                    ...
                    LogWarning("error decompressing obj %d: %s" % (obj_num, e))
        ...
        if counts:
            ...
            msg = "none"
        LogInfo("%s %d streams, kept %s" % (what, len(self.objs) - skipped_count, msg))
    ...
    def DecompressStreams(self, is_flate_only):
        ...
        for pdf_obj in self.objs.values():
            if pdf_obj.head.startswith("<<") and substring in pdf_obj.head:
                ...
                if isinstance(
                    filter_value, str
                ):  # Should always be true (except None).
                    if is_flate_only:
                        do_decompress = "/FlateDecode" in filter_value
                    else:
                        do_decompress = (
                            filter_value
                            and "/DCTDecode" not in filter_value
                            and "/JPXDecode" not in filter_value
                        )
                    if do_decompress:
    ...
    def CompressUncompressedStreams(self):
        ...
        for pdf_obj in self.objs.values():
            if (
                pdf_obj.stream is not None
                and pdf_obj.head.startswith("<<")
                and pdf_obj.Get("Filter") in (None, "[]")
            ):
    ...
    def ParseSequentially(
        self,
        data,
        file_name=None,
        offsets_out=None,
        obj_num_by_ofs_out=None,
        ...
        while 1:
            ...
            try:
                pdf_obj = PdfObj(
                    data, start=i, end_ofs_out=end_ofs_out, file_ofs=i, objs=length_objs
                )
    ...
    def ComputePdfStatistics(cls, file_name):
        ...
        def SetItemCallback(obj_num, pdf_obj, end_ofs):
            if obj_num is None:
                ...
                if end_ofs in ("header", "linearized_xref", "trailer", "xref"):
            ...
            if pdf_obj.head.startswith("<<"):
                if pdf_obj.Get("Type") == "/ObjStm":
                    ...
                    for head in PdfObj.ParseArray("[%s]" % obj_data):
                        if isinstance(head, str) and head.startswith("<<"):
                            ...
                            if (
                                dict_obj.get("Type") == "/Page"
                                and "Contents" in dict_obj
                            ):
                    ...
                    pdf_obj.Get("Type") == "/Page"
                    and pdf_obj.Get("Contents") is not None
                ):
                ...
                # objs=pdf.objs below.
                if pdf_obj.Get("Subtype") == "/Image" or pdf_obj.DetectInlineImage(
                    objs=pdf.objs
                ):
        ...
        for obj_nums1 in obj_num_arys:
            for obj_nums2 in obj_num_arys:
                assert obj_nums1 is obj_nums2 or not obj_nums1.intersection(
                    obj_nums2
        ...
        assert not [
            1 for value in stats.values() if value < 0
        ], "stats has negative values"
        sum_stats = sum(stats.values())
        assert sum_stats == len(
            data
    ...
    def FixPdfFromMultivalent(
        ...
        assert (
            do_generate_xref_stream or not do_generate_object_stream
        ...
        pdf = cls().ParseSequentially(  # PdfData().
            data=data, offsets_out=in_offsets, obj_num_by_ofs_out=obj_num_by_in_ofs
        )
        assert pdf.trailer.stream is not None
        if not is_flate_ok:
        ...
        # emit the trailer.
        assert (
            pdf_objs[trailer_obj_num] is trailer_obj
        ...
        for offsets_idx in range(len(in_offsets) - 1):
            ...
            if type(obj_num) not in (int, int):
                raise PdfTokenParseError
            if obj_num < 1:
                raise PdfTokenParseError
        ...
        for offsets_idx in range(in_offsets_limit):
            ...
            if obj_out_size > obj_size and is_flate_ok:
                raise PdfOptimizeError(
                    "size of obj %s has grown from %s to %s bytes"
        ...
        if do_generate_xref_stream and bool(do_generate_object_stream) == bool(
            has_objstm_obj
        ):
        ...
        if do_generate_object_stream and not has_objstm_obj:
            ...
            assert not (
                not do_generate_object_stream
                and has_objstm_obj
                and not compressed_objects
            ...
            if not do_generate_object_stream and compressed_objects:
                ...
                for ref_obj_num in sorted(compressed_objects):
                    ...
                    if objstm_items is None:
                        compressed_obj_nums, compressed_obj_headbufs = objstm_objs[
                            f1
                        ].ParseObjStm(ref_obj_num)
                    ...
                    compressed_pdf_obj.AppendTo(
                        output, ref_obj_num, do_emit_short_unsafe=True
                    )
            ...
            if do_generate_xref_stream:
                ...
                if not do_generate_object_stream and has_objstm_obj:
                    ...
                    cls.GenerateXrefStream(
                        obj_numbers=sorted(out_ofs_by_num),
                        xref_ofs=xref_ofs,
                        trailer_obj_num=out_trailer_obj_num,
                        trailer_obj=trailer_obj,
                        obj_ofs=out_ofs_by_num,
                        objstm_obj_num=None,
                        objstm_obj_numbers=None,
                        is_flate_ok=is_flate_ok,
                    )
                    ...
                    trailer_obj.SetStreamAndCompress(
                        xref_out,
                        may_keep_old=(xref_out == xref_data and is_flate_ok),
                        predictor_width=(w0 + w1 + w2),
                        is_flate_ok=is_flate_ok,
                    )
                LogInfo(
                ...
                trailer_obj.AppendTo(
                    output, out_trailer_obj_num, do_emit_short_unsafe=True
                )
                while output_size_idx < len(output):
                    output_size += len(output[output_size_idx])
                    output_size_idx += 1
        ...
        if (
            ...
            and output_size > len(data)
            and (do_generate_object_stream or not has_objstm_obj)
            and is_flate_ok
        ):
            ...
            raise PdfOptimizeError(
                "PDF size has grown from %s to %s bytes" % (len(data), output_size)
            )
        return output_size

    PDFDATA_MULTIVALENT_EXT_SUB_RE = re.compile(r"[.][^.]+\Z")

    def _RunMultivalent(self, do_escape_images, multivalent_compress_command):
        ...
        out_pdf_tmp_file_name = (
            self.PDFDATA_MULTIVALENT_EXT_SUB_RE.sub("", in_pdf_tmp_file_name) + "-o.pdf"
        )
        ...
        in_data_size = self.AppendSerializedPdf(
            output=tmp_output,
            do_hide_images=do_escape_images,
            do_generate_xref_stream=True,
            do_generate_object_stream=True,
        )
        ...
        LogInfo(
            ...
            % (
                out_pdf_tmp_file_name,
                out_data_size,
                FormatPercent(out_data_size, in_data_size),
            )
        )
        assert out_data_size, "Multivalent generated empty output (see its error above)"
    ...
    def Save(
        ...
        estimated_size = (
            40 + self.trailer.size + sum(pdf_obj.size for pdf_obj in self.objs.values())
        )
        ...
        for job in jobs:
            ...
            if multivalent_compress_command:
                ...
                output_size = self.AppendSerializedPdf(output=output, **job[0])
            if len(jobs) > 1:
                LogInfo(
...
def ParseUintFlag(key, flag_value):
    ...
    try:
        ...
        raise getopt.GetoptError(
            "flag %s=%s needs a nonnegative integer value" % (key, flag_value)
        )
    return value
...
def DisplayHelp(mode, argv0):
    LogInfo("usage for statistics computation: %s --stats <input.pdf>" % argv0)
    LogInfo(
...
def main(argv, script_dir=None, zip_file=None):
    ...
    try:
        ...
        if f.mode == "stats":
            ...
            if not [
                1 for cmd_pattern in f.img_cmds if GetCmdName(cmd_pattern) == "sam2p_np"
            ]:
            ...
            if f.do_generate_object_stream and not f.do_generate_xref_stream:
                raise getopt.GetoptError(
                    "--do-generate-object-stream=yes requires "
    ...
    if f.do_debug_gs:
        ...
        LogInfo("found working Ghostscript: %s" % GetGsCommand(is_verbose=True))

    if not f.use_multivalent:
        multivalent_compress_command = None
    else:
        multivalent_compress_command = GetMultivalentCompressCommand(libexec_dir)
    ...
    if (
        f.do_compress_uncompressed_streams
        and not f.do_decompress_most_streams
        and multivalent_compress_command is None
    ):
    ...
    pdf.Save(
        ...
        is_flate_ok=(
            f.do_compress_uncompressed_streams and not f.do_decompress_most_streams
        ),
    )
